...
export default function CalendarAdmin() {
  ...
  const [booked, setBooked] = useState(new Map()) // date -> { id, inquiry_id, navn, ... }
  const [busy, setBusy] = useState(false)
  const [version, setVersion] = useState(0)
  const [selected, setSelected] = useState(null) // { date, ...info } eller null
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const { data } = await supabase
        .from('booked_dates')
        .select('id, date, inquiry_id, inquiries(name, phone, email, event_type, guests)')
      if (!active) return
      const map = new Map(
        (data || []).map((r) => [
          r.date,
          { id: r.id, inquiry_id: r.inquiry_id, ...(r.inquiries || {}) },
        ]),
      )
      setBooked(map)
    })()
  ...
  function onDayClick(day) {
    ...
    if (info) {
      // Opptatt: vis hvem som har datoen i stedet for å fjerne den direkte.
      setSelected({ date: dateStr, ...info })
    } else {
      bookManually(dateStr)
    }
  }

  // Klikk på ledig dato → marker som opptatt manuelt (uten forespørsel).
  async function bookManually(dateStr) {
    setBusy(true)
  ...
  return (
    <div>
      ...
      <p className="mb-6 text-sm text-ink-light">
        Klikk på en ledig dato for å sette den opptatt. Klikk på en opptatt dato for å
        se hvem som har den.
      </p>
        ...
        >
          ...
          >
            ...
            <h3 className="mt-1 text-xl text-primary">
              {new Date(selected.date).toLocaleDateString('no-NO', {
                weekday: 'long',
                day: 'numeric',
                month: 'long',
                year: 'numeric',
            ...
            {selected.name ? (
              ...
              <p className="mt-5 text-sm text-ink-light">
                Datoen er satt opptatt manuelt – ingen forespørsel er knyttet til den.
              </p>
            )}
